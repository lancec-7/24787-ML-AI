...
def feature_engineer_normalization(trainX):
    # mu = np.mean(trainX,axis=0)
    # sigma = np.sqrt(np.mean((trainX - mu)**2,axis=0))
    # trainX_new = trainX / sigma

    trainX_new = np.log(trainX)

    # max = np.max(trainX,axis=0)
    # min = np.min(trainX,axis=0)
    # scale = 1/(max-min)
    # trainX_new = scale * (trainX - min)
    return trainX_new

#  use the polynomial
def feature_engineer(trainX,n):
    trainX_new = np.empty((trainX.shape[0],n*trainX.shape[1]))

    for column in range(trainX.shape[1]):

        trainX_new[:,n*column:n*(column+1)] = np.array([trainX[:,column]**(i+1) for i in range(n)]).T
    return trainX_new
...
class LinearRegression:
    ...
    def fit(self,trainX,trainY,lr=1e-6,max_iter=10000):
        pass
...
def main():
    ...
    trainX_new = feature_engineer(trainX,5)
    trainX_new = feature_engineer_normalization(trainX_new)
    print(trainX_new[0,:])

    # testX_new = feature_engineer(testX,5)
